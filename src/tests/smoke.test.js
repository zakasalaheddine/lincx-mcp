...
test('smoke > test harness boots', async t => {
  const res = await request(buildContractApp()).get('/health')
  t.is(res.status, 200)
  t.is(res.body.status, 'ok')
})

test('smoke > serves authorization-server metadata', async t => {
  const res = await request(buildContractApp()).get('/.well-known/oauth-authorization-server')
  t.is(res.status, 200)
...
test('smoke > serves protected-resource metadata', async t => {
  const res = await request(buildContractApp()).get('/.well-known/oauth-protected-resource')
  t.is(res.status, 200)
...
test('smoke > serves protected-resource metadata at the RFC 9728 path-scoped URL', async t => {
  const res = await request(buildContractApp()).get('/.well-known/oauth-protected-resource/mcp')
  t.is(res.status, 200)
  t.regex(res.body.resource, /\/mcp$/)
  t.true(res.body.authorization_servers instanceof Array)
})
