...
export async function getEntityWithIncludes(
  ...
  if (!include?.includes("parents")) {
    return workApiRequest<unknown>(session, "GET", `${basePath}/${id}`);
  }
  const [entity, parents] = await Promise.all([
    workApiRequest<unknown>(session, "GET", `${basePath}/${id}`),
    workApiRequest<unknown>(session, "GET", `${basePath}/${id}/parents`),
  ]);
