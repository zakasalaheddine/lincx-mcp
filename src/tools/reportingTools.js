...
export function registerReportingTools (server) {
  ...
  server.registerTool('list_dimension_sets', {
    ...
    try {
      const data = await workApiRequest(v.session, 'GET', '/api/dimension-sets', { params: { limit, offset } })
  ...
  server.registerTool('get_dimension_set', {
    ...
    try {
      const data = await workApiRequest(v.session, 'GET', `/api/dimension-sets/${id}`)
  ...
  server.registerTool('get_event_stats_keys', {
    ...
    try {
      const data = await workApiRequest(v.session, 'GET', '/api/event-stats')
      const text = JSON.stringify(data)
      return { content: [{ type: 'text', text: truncateIfNeeded(text) }] }
    } catch (err) {
      return { content: [{ type: 'text', text: handleWorkApiError(err) }] }
...
- Only pass raw:true if you genuinely need every hourly row (large; may be truncated).`,
    ...
    if (timezone && !isValidTimeZone(timezone)) {
      return { content: [{ type: 'text', text: `Error: '${timezone}' is not a valid IANA timezone (e.g. 'America/Denver', 'UTC').` }] }
    }
    ...
    try {
      ...
      const params = { startDate: fetchStart, endDate: fetchEnd }
      ...
      if (missingKeys.length) {
        return { content: [{ type: 'text', text: `Error: filter key(s) [${missingKeys.join(', ')}] are not dimensions of this set. Call get_dimension_set to see valid dimensions, or use one in groupBy.` }] }
      }
      if (filter && fetchedRows.length > 0 && rows.length === 0) {
        const hint = valueHints.length ? ` Values present in range: ${valueHints.join(' | ')}.` : ''
        return { content: [{ type: 'text', text: `No rows matched filter ${JSON.stringify(filter)} over ${startDate}→${endDate}.${hint}` }] }
      }
...
export function isValidTimeZone (tz) {
  try {
    // eslint-disable-next-line no-new -- constructing it IS the validity check; it throws on a bad zone
