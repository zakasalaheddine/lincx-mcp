...
- [x] **T3-2 — Heavy fields can never leak into a list.** DONE — `list_templates` (like all `list_*`) goes through `buildListEnvelope` → `projectListItem`, whose default `{id,name}`+status projection already excludes `html/css/...`. The one new leak path was the T2-6 `fields:['*']` escape hatch returning the raw row; closed it — `['*']` now returns `stripHeavyFields(row)`, so full-row mode still drops content blobs (they belong in `get_*`). Tested in `src/tests/listEnvelope.test.ts`.
- [x] **T3-3 — `report_query` raw mode is now bounded.** DONE — `raw:true` caps rows to a char budget so the whole result clears the 30k guard instead of being nuked by it. Subtlety handled: `outputSchema` forces `structuredContent`, so rows appear ~2.4× in the serialized result (raw in structuredContent + escaped-as-string in the content text) — budget is `(RESPONSE_SIZE_LIMIT-2000)/2.5`. When capped, returns a `rawTruncated {returned,total,note}` field (added to the outputSchema) pointing to aggregated mode / a narrower range. The aggregation path itself (`aggregateReport`) is unchanged — still the model to copy for high-row endpoints. Tested (`reportQuery.structured.test.ts`: 5000-row input stays ≤30k, validates against outputSchema).
